            ...
            uber: function (clientId, appScope, options) {
                ...
                if (window.cordova) {
                    ...
                    if ($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true) {
                        ...
                        console.log('https://login.uber.com/oauth/v2/authorize?' +
                            'redirect_uri=' + encodeURIComponent(redirect_uri) +
                            '&response_type=code' +
                            '&scope=' + appScope.join(" ") +
                            '&client_id=' + clientId);

                        var browserRef = window.open('https://login.uber.com/oauth/v2/authorize?' +
                            'redirect_uri=' + encodeURIComponent(redirect_uri) +
                            '&response_type=code' +
                            '&scope=' + appScope.join(" ") +
                            '&client_id=' + clientId, '_blank', 'location=no,clearsessioncache=yes,clearcache=yes');
